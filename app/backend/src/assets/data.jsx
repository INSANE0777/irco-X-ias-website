import { TbBeach, TbMountain, TbPool } from "react-icons/tb";
import {
  GiBoatFishing,
  GiCactus,
  GiFamilyHouse,
  GiOfficeChair,
  // GiIsland,
  GiWindmill,
} from "react-icons/gi";
import {
  // FaSkiing,
  FaPumpSoap,
  FaShower,
  FaFireExtinguisher,
  // FaHouse,
  FaUmbrellaBeach,
  FaKey,
  FaBusinessTime,
  FaHome,
  FaUsers,
} from "react-icons/fa";
import { FaKitchenSet, FaBed } from "react-icons/fa6";
import {
  BiSolidWasher,
  BiSolidDryer,
  BiSolidFirstAid,
  BiWifi,
  BiSolidFridge,
  BiWorld,
} from "react-icons/bi";
import { BsSnow, BsPersonWorkspace } from "react-icons/bs";
import {
  MdOutlineVilla,
  MdMicrowave,
  MdBalcony,
  MdOutlinePool,
  MdYard,
  MdOutlineApartment,
  MdPets,
  MdTempleHindu,
} from "react-icons/md";
import {
  PiBathtubFill,
  PiFarmBold,
  PiCoatHangerFill,
  PiTelevisionFill,
} from "react-icons/pi";
import { TbIroning3 } from "react-icons/tb";
import {
  GiHeatHaze,
  GiCctvCamera,
  
  GiBarbecue,
  GiToaster,
  GiCampfire,
} from "react-icons/gi";
import { AiFillCar } from "react-icons/ai";
import { CiShop } from "react-icons/ci";

export const categories = [
  {
    label: "All",
    icon: <BiWorld />,
    color: "#bfdbfe", // blue-200
  },
  {
    label: "Apartment",
    icon: <MdOutlineVilla />,
    color: "#ffe4b5", // light beige
  },
  {
    label: "Plot",
    icon: <PiFarmBold />,
    color: "#e9d5ff", // purple-200
  },
  {
    label: "Farm",
    icon: <GiWindmill />,
    color: "#d1fae5", // green-200
  },
  {
    label: "Rural Area",
    icon: <TbMountain />,
    color: "#ccfbf1", // teal-200
  },
  {
    label: "Condo",
    icon: <MdOutlineApartment />, // Example icon
    color: "#e5e7eb", // gray-200
  },
  {
    label: "House",
    icon: <FaHome />,
    color: "#e0e7ff", // indigo-200
  },
  {
    label: "Office Space",
    icon: <GiOfficeChair />,
    color: "#fef3c7", // yellow-200
  },
  {
    label: "Luxury house",
    icon: <GiFamilyHouse />,
    color: "#cffafe", // cyan-200
  },
  {
    label: "Shop",
    icon: <CiShop />,
    color: "#bfdbfe", // blue-200
  },
];

export const types = [
  {
    name: "Commercial",
    description: "Commercial land is designated for business activities aimed at generating profit.",
    icon: <FaBusinessTime />,
  },
  {
    name: "Residential",
    description: "Residential land is designated for housing purposes.",
    icon: <FaHome />,
  },
  {
    name: "Industrial",
    description:
      "Industrial land is designated for industrial activities, including manufacturing, processing, warehousing, and distribution.",
    icon: <GiFamilyHouse />,
  },
  {
    name: "Agricultural",
    description: "Agricultural land is designated to the cultivation of crops and the rearing of livestock.",
    icon: <FaUsers />,
  },
];

export const facilities = [
  {
    name: "Luxury Bathtub",
    icon: <PiBathtubFill />,
  },
  {
    name: "Premium Care Essentials",
    icon: <FaPumpSoap />,
  },
  {
    name: "Al Fresco Shower",
    icon: <FaShower />,
  },
  {
    name: "Efficient Washer",
    icon: <BiSolidWasher />,
  },
  {
    name: "High-Performance Dryer",
    icon: <BiSolidDryer />,
  },
  {
    name: "Clothing Hangers",
    icon: <PiCoatHangerFill />,
  },
  {
    name: "Ironing Essentials",
    icon: <TbIroning3 />,
  },
  {
    name: "Smart TV",
    icon: <PiTelevisionFill />,
  },
  {
    name: "Home Office Space",
    icon: <BsPersonWorkspace />,
  },
  {
    name: "Climate Control - AC",
    icon: <BsSnow />,
  },
  {
    name: "Central Heating",
    icon: <GiHeatHaze />,
  },
  {
    name: "Surveillance Cameras",
    icon: <GiCctvCamera />,
  },
  {
    name: "Safety Fire Extinguisher",
    icon: <FaFireExtinguisher />,
  },
  {
    name: "Emergency First Aid Kit",
    icon: <BiSolidFirstAid />,
  },
  {
    name: "High-Speed Wifi",
    icon: <BiWifi />,
  },
  {
    name: "Complete Cooking Set",
    icon: <FaKitchenSet />,
  },
  {
    name: "Large Refrigerator",
    icon: <BiSolidFridge />,
  },
  {
    name: "Microwave Oven",
    icon: <MdMicrowave />,
  },
  {
    name: "Electric Stove",
    icon: <GiToaster />,
  },
  {
    name: "Outdoor Pool",
    icon: <MdOutlinePool />,
  },
  {
    name: "Open-Air Dining",
    icon: <FaUmbrellaBeach />,
  },
  {
    name: "Private Terrace/Balcony",
    icon: <MdBalcony />,
  },
  {
    name: "Pooja Space",
    icon: <MdTempleHindu />,
  },
  {
    name: "Lush Garden",
    icon: <MdYard />,
  },
  {
    name: "Complimentary Parking",
    icon: <AiFillCar />,
  },
  {
    name: "Contactless Check-In",
    icon: <FaKey />,
  },
  {
    name: "Pet-Friendly Space",
    icon: <MdPets />,
  },
];
